...
class Board {
    ...
    fun computerTurn() {
        if(isComputerTurn()) {
            ...
            println(curPlayer.getColorString().toString() + " (" + aiCoord[0] + "; " + aiCoord[1] + ")" + "\t move uses card " + aiCard)
        }
    }
    ...
    private fun alphabeta() {
        ...
        println("Selected move score: " + alphabeta(this, aiCurPlayer.getDifficulty(), Int.MIN_VALUE, Int.MAX_VALUE, true))
    }
            ...
            for(c in node.getCardsOfPlayerColor(aiCurPlayer.getColor())) {
                for(p in node.getPiecesOfColor(aiCurPlayer.getColor())) {
                    ...
                    for(move in c.getMoves()) {
                        ...
                        if(!to.validateMove(move[0], move[1], aiCurPlayer.getColor())) {
                            continue
                        }
                        if(child.getPiece(from) == null) {
                            continue
                        }
                        if(child.getPiece(to) != null && child.getPiece(from)!!.color == child.getPiece(to)!!.color) {
                            continue
                        }
                        child.play(from, to)
                        child.swapCard(c)
            ...
            for(c in node.getCardsOfPlayerColor(aiOffPlayer.getColor())) {
                for(p in node.getPiecesOfColor(aiOffPlayer.getColor())) {
                    ...
                    for(move in c.getMoves()) {
                        ...
                        val from = Coordinate(p.coordinate.x, p.coordinate.y)
                        val to = Coordinate(from.x, from.y)
                        if(!to.validateMove(move[0], move[1], aiOffPlayer.getColor()) || from == to) {
                            continue
                        }
                        if(child.getPiece(from) == null) {
                            continue
    ...
    fun getWinner(): Player? {
        if(blueWin || !containsRedKing) {
            return getPlayerOfColor(Piece.COLOR_BLUE)
        }
        return if(redWin || !containsBlueKing) {
            getPlayerOfColor(Piece.COLOR_RED)
        } else {
            null
        }
    }
    ...
    fun checkWin(): Boolean {
        ...
        if(p != null && p.isKing && p.color == Piece.COLOR_BLUE) {
            return true.also {
                blueWin = it
            }
        }
        p = getPiece(Coordinate.BLUE_CROWN)
        if(p != null && p.isKing && p.color == Piece.COLOR_RED) {
            return true.also {
                redWin = it
            }
        }
    ...
    fun printBoard() {
        for(i in 0..4) {
            for(j in 0..4) {
                System.out.printf("|%2s ", if(board[i][j] == null) {
                    ""
                } else if(board[i][j]!!.color == Piece.COLOR_RED) {
                    "r"
                } else {
                    "b"
                })
    ...
    fun printBoard(child: Array<Array<Piece?>>) {
        for(i in 0..4) {
            for(j in 0..4) {
                System.out.printf("|%2s ", if(child[i][j] == null) {
                    ""
                } else if(child[i][j]!!.color == Piece.COLOR_RED) {
                    "r"
                } else {
                    "b"
                })
    ...
    private fun getBoardString(): String {
        var ret = ""
        for(i in 0..24) {
            ret += "|" + i % 5 + (if(getPiece(i % 5, i / 5) == null) {
                " "
            } else if(getPiece(i % 5, i / 5)!!.color == Piece.COLOR_RED) {
                "r"
            } else {
                "b"
            }) + i / 5 + if(i % 5 == 4) {
